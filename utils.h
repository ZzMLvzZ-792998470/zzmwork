...
public:
    //合并所有视频帧到一个帧并输出
    static AVFrame* merge_way(int& backround_height, int& backround_width, std::vector<AVFrame *>& frames);

    //改变数据帧画面大小
    static AVFrame* change_frame_size(int new_width, int new_height, AVFrame *src_frame);

    //AVFrame ----> Mat
    static cv::Mat avframe2Mat(int& height, int& width, AVFrame *frame);
